...
public:
    DRV2605(void);
    boolean begin(TwoWire *theWire = &Wire, uint8_t sdaPin = 21, uint8_t sclPin = 22);

    boolean init(uint8_t sdaPin = 21, uint8_t sclPin = 2);
    ...
    void useERM();
    void useLRA(float v_rms, float v_peak, float f_res);
    void autoCalibrate();
    void setupLRA(TwoWire* the_wire, int sdaPin, int sclPin);
